...
1. **FastAPI (백엔드 호스트)**:
   ...
   * **Disk**: 파일 업로드로 인한 디스크 잔여량 확보를 관제하기 위해, 로컬 개발 환경에서는 현재 작업 디렉터리(`.`) 기준의 실제 디스크 여유 공간을 기가바이트(GB) 및 진행 바로 표시합니다. Render 운영 환경(production)에서는 임시(Ephemeral) 컨테이너 스토리지만 제공되므로 디스크 실측이 무의미하고 호스트 대형 볼륨(386GB) 노출 왜곡을 막기 위해 디스크 수집 값을 0(0 GB / 0.0%)으로 강제 마스킹하여 표시합니다.
...
5. **운영 환경 디스크 노이즈 차단 및 0 마스킹 처리**:
   * 로컬 개발 환경에서는 디스크 계측 경로를 시스템 루트(`/`)에서 작업 디렉터리(`.`)로 전환하여 불필요한 호스트 드라이브 조회를 차단했습니다.
   * Render 무료 플랜의 임시 컨테이너 특성상 영구 스토리지가 지원되지 않고 호스트 볼륨(386GB)이 왜곡되어 노출되는 문제를 방지하기 위해, 배포 환경(`APP_ENV/ENV_MODE == "production"`)에서는 디스크 수집을 제외하고 용량 및 퍼센트를 0(0 GB / 0.0%)으로 강제 마스킹 처리하여 대시보드 지표의 정확성과 정체성을 확립했습니다.
