...
1. **FastAPI (백엔드 호스트)**:
   ...
   * **Disk**: 파일 업로드로 인한 디스크 잔여량 확보를 관제하기 위해, 로컬 개발 환경에서는 현재 작업 디렉터리(`.`) 기준의 실제 디스크 여유 공간을 기가바이트(GB) 및 진행 바로 표시합니다. Render 운영 환경(production)에서는 리눅스 컨테이너 가상화 격리 특성상 호스트 본체 볼륨(386GB) 노이즈가 노출되는 왜곡 현상을 방지하고자 1GB 규격의 고정 더미 데이터(사용량 100MB, 10.0%)로 강제 마스킹하여 대시보드의 일관성을 보장합니다.
...
5. **운영 환경 디스크 노이즈 차단 및 마스킹 방어**:
   * 로컬 개발 환경에서는 디스크 계측 경로를 시스템 루트(`/`)에서 작업 디렉터리(`.`)로 전환하여 불필요한 호스트 드라이브 조회를 차단했습니다.
   * Render 컨테이너의 격리 한계로 인해 공유 호스트 볼륨(386GB)이 렌더링되는 왜곡 현상을 원천 방어하기 위해, 배포 환경(`APP_ENV == "production"`)인 경우 디스크 모듈 실측을 차단하고 `1GB` 총량에 `100MB 사용 (10.0%)` 형태의 마스킹 규격을 강제로 태워 대시보드 무결성을 확보했습니다.
