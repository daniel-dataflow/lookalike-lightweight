...
2. **Neon PostgreSQL (이중 분할 DB 용량 모니터링 고도화)**:
   * 단순 접속 생존 유무 확인에 그치지 않고, **로컬/개발용(DEV) 세트와 상용/실서버(PROD) 세트를 카드 내에서 완벽하게 시각적으로 격리**하여 표기합니다.
   * **DEV 세트**: `DEV_DB` (개발 메인 DB), `DEV_DW_DB` (개발 수집/로그 DB)의 실시간 크기를 MB 단위로 조회하고, 이를 합산한 `DEV 합계 용량`을 동적으로 나타냅니다.
   * **PROD 세트**: `PROD_DB` (실제 서비스 DB), `PROD_DW_DB` (실제 서비스 수집/로그 DB)의 실시간 크기 및 합계 용량을 동일한 체계로 산출합니다.
   * Neon DB API 및 데이터베이스 카탈로그 쿼리를 병행하여, 무료 티어의 치명적인 용량 상한을 관리자가 상시 인지하고 자원 배치를 조정할 수 있도록 계측 통계를 제공합니다.

3. **Cloudinary (이미지 저장소)**:
   * 기존 MongoDB 카드를 대체합니다.
   * Cloudinary API를 이용해 총 미디어 보관 크기(GB 단위 변환 표시)와 업로드된 이미지 리소스의 개수 및 크레딧(Credit) 사용 비율을 렌더링합니다.

4. **HuggingFace Space (ML 모델 호스팅)**:
   * 기존 Redis 카드를 대체합니다.
   * 모델 서빙 API 상태를 동적으로 호출 및 분석하고, 모델 응답 속도(Latency, ms) 및 Git Storage 사용량을 실시간으로 측정하여 통신 상태가 정상인지 식별합니다.
...
7. **Cloudinary 지표 렌더링 수치 음수 보정 처리**:
   * Cloudinary API 캐시 지연이나 자동 동기화 주기로 인해 드물게 일시적으로 리소스 수량, 사용 디스크 크기, 혹은 사용 크레딧 수치에 음수(-) 값이 반환될 수 있습니다.
   * UI 대시보드 상에 마이너스 기호가 그대로 노출되어 생기는 어색함을 제거하기 위해 프론트엔드 모니터링 코드(`admin_infra.js`)의 데이터 바인딩 로직에 `Math.max(0, ...)`를 씌워 최소 0 GB, 0개, 0 Credit으로 표기되도록 프론트엔드 예외 처리를 추가 적용했습니다.
