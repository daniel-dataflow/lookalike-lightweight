#!/usr/bin/env python
"""
init_test_db.py - TEST_DATABASE_URL에 필수 테이블(products, naver_prices, product_embeddings 등)을 생성하는 초기화 스크립트
"""
import os
import sys
import psycopg2
...
def main():
    # .env 파일 로딩을 위해 직접 파싱 수행 (base_utils와 유사하게)
    db_url = None
    ...
    if not db_url:
        print("[ERROR] TEST_DATABASE_URL 환경 변수가 .env 또는 시스템에 설정되지 않았습니다.")
        sys.exit(1)
        
    print(f"[INFO] TEST DB init start (Target Host: {urlparse(db_url).hostname})")
    ...
    try:
        ...
        print("[SUCCESS] TEST_DATABASE_URL DB tables and indexes initialized successfully!")
